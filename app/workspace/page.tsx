...
type WorkspaceProject = {
  ...
  contactName: string;
  email: string;
  messages: Array<{ id: string; sender: "filvon" | "client"; name: string; text: string; time: string }>;
  feedback: Array<{ id: string; title: string; category: string; status: "Open" | "In behandeling" | "Afgerond" }>;
  ...
  useEffect(() => {
    async function loadWorkspace() {
      ...
      if (mappingError || !mapping) {
        setProjectError("Geen klantprofiel gekoppeld aan dit account.");
      ...
      const { data: clientData, error: clientError } = await supabase
        .from("clients")
        .select("id, client_name, project_name, status, progress, deadline, last_update, preview_url, contact_name, email")
        .eq("id", mapping.client_id)
        .single();

      if (clientError || !clientData) {
        setProjectError("Klantgegevens konden niet geladen worden.");
        setLoadingProject(false);
        return;
      }
...
setProject({
  ...
  status: clientData.status ?? "Discovery",
  progress: Number(clientData.progress ?? 0),
  deadline: clientData.deadline ?? "",
  lastUpdate: clientData.last_update ?? "",
  previewUrl: clientData.preview_url ?? "",
...
setFeedbackItems(
  (feedbackData ?? []).map((item) => ({
    ...
    status: item.status as "Open" | "In behandeling" | "Afgerond",
  })),
);
setLoadingProject(false);
}
...
if (loadingProject) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#05080E] text-white">
...
if (projectError || !project) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#05080E] px-5 text-white">
      <div className="max-w-[520px] text-center">
        <p className="text-[10px] uppercase tracking-[0.3em] text-blue-200/55">
          FILVON Client Portal
        </p>

        <h1 className="mt-4 text-3xl font-semibold">
          Workspace niet beschikbaar
        </h1>

        <p className="mt-4 text-sm leading-7 text-white/40">
          {projectError || "Project kon niet geladen worden."}
        </p>

        <button
          type="button"
          onClick={handleLogout}
          className="mt-6 h-11 rounded-lg border border-white/10 bg-white/[0.035] px-5 text-sm font-semibold text-white"
        >
          Terug naar login
        </button>
      </div>
  ...
  async function handleChatSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        sender: "client",
        sender_name: project?.contactName?.split(" ")[0] || "Klant",
        message,
      })
      .select("id, sender, sender_name, message, created_at")
      .single();
    ...
    setFeedbackItems((current) => [
      {
        ...
        status: data.status as "Open" | "In behandeling" | "Afgerond",
      },
      ...current,
    ]);
        ...
        <div className="mx-auto flex h-[82px] max-w-[1540px] items-center justify-between px-5 sm:px-8 lg:px-[64px]">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              onClick={handleLogout}
              disabled={loggingOut}
              className="hidden h-10 items-center justify-center rounded-lg border border-white/10 bg-white/[0.035] px-4 text-xs font-semibold text-white/70 transition hover:border-blue-400/30 hover:text-white disabled:opacity-40 sm:inline-flex"
            >
            ...
            <div className="flex h-11 w-11 items-center justify-center rounded-full border border-blue-400/25 bg-blue-500/[0.08] text-sm font-semibold text-blue-100">
              {(project.contactName?.[0] || project.clientName?.[0] || "K").toUpperCase()}
            </div>
          </div>
        </div>
      </header>
        ...
        <section className="flex flex-col gap-8 border-b border-white/10 pb-10 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-5 text-[clamp(2.4rem,4vw,4.6rem)] font-semibold leading-[0.96] tracking-[-0.05em]">
              Welkom terug,
              <br />
              <span className="text-blue-400">{project.contactName?.split(' ')[0] || project.clientName}.</span>
            </h1>
          ...
          >
            ...
            <div className="relative flex h-full flex-col">
              ...
              <a
                ...
                rel={project.previewUrl ? "noreferrer" : undefined}
          ...
          >
            ...
            <div className="mt-8 space-y-3">
              {feedbackItems.length === 0 && (
                <div className="rounded-2xl border border-white/10 bg-[#07101D] p-5 text-sm text-white/35">Nog geen feedback voor dit project.</div>
              )}
          ...
          <div className="rounded-[28px] border border-white/[0.13] bg-[#0A101A] p-6 sm:p-8">
            ...
            <div className="mt-8 space-y-3">
              {project.files.length === 0 && (
                <div className="rounded-2xl border border-white/10 bg-[#07101D] p-5 text-sm text-white/35">Nog geen bestanden beschikbaar.</div>
              )}
          ...
          <div className="overflow-hidden rounded-[28px] border border-white/[0.13] bg-[#0A101A]">
            ...
            <div className="space-y-5 p-6 sm:p-8">
              {chatMessages.length === 0 && (
                <div className="rounded-2xl border border-white/10 bg-[#07101D] p-5 text-sm text-white/35">Nog geen berichten in dit projectgesprek.</div>
              )}
            ...
            >
              ...
              <p className="mt-3 text-xs leading-5 text-white/25">
                Berichten worden in een volgende stap gekoppeld aan de projectdatabase.
              </p>
            </form>
          </div>
